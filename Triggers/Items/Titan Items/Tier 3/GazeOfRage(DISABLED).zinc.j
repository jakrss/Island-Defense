...
library GazeOfRage requires ItemExtras, BonusMod, BUM {
    ...
    function damageExpire() {
        timer t = GetExpiredTimer();
        integer th = GetHandleId(t);
        unit caster = LoadUnitHandle(gazeTable, 0, th);
        item i = LoadItemHandle(gazeTable, 1, th);
        
        BlzItemRemoveAbility(i, DMG_ID);
        DestroyTimer(t);
        FlushChildHashtable(gazeTable, th);
    ...
    function onCast() {
        ...
        integer aLvl = GetUnitAbilityLevel(caster, DMG_ID);
        timer t = LoadTimerHandle(gazeTable, 1, GetHandleId(caster));
        integer th = GetHandleId(t);
        item i = GetItemFromUnitById(caster, ITEM_ID);
        
        if(t == null) {
            t = CreateTimer();
            th = GetHandleId(t);
            
            SaveUnitHandle(gazeTable, 0, th, caster);
            SaveItemHandle(gazeTable, 1, th, i);
            
            BlzItemAddAbility(i, DMG_ID);
            TimerStart(t, DMG_DUR, false, function damageExpire);
            
            SaveTimerHandle(gazeTable, 1, GetHandleId(caster), t);
        } else {
            DestroyTimer(t);
            FlushChildHashtable(gazeTable, th);
