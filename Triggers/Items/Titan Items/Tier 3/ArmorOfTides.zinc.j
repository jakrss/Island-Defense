//! zinc
library ArmorOfTides requires BonusMod, ItemExtras, BUM {
    //Item ID of Super Regenerative Spines
    private constant integer ITEM_ID = 'I06E';
    //Ability ID
    ...
    function regenPercent(unit u, real curHP, real maxHP) {
        addHealth(u, maxHP * P_LIFE * TIMER_SPEED);
        if(GetUnitAbilityLevel(u, ABILITY_ID) == 1) {
            IncUnitAbilityLevel(u, ABILITY_ID);
        }
    }
