...
library TitanHunterExperience {
	//Generic:
	private constant integer CritterXP = 2; // 2 = 2 % for Every Level 4 = 4% for every level 8 = 8% etc
	private constant integer UnitXP = 25;	
	private constant integer HeroXP = 350;	
	private constant real XPSpillRange = 1500;
	private constant real dmgXexpFactor = 0.0015;
	private hashtable XPHash = InitHashtable();

	public function addExp(unit u, integer xp) {
    ...
    private function onInit(){
		...
		TriggerAddCondition(t, Condition(function() -> boolean {
			...
			//Killing Critters: CHANGED THIS TO BE INCREASE WITH LEVELS
			if(GetUnitAbilityLevel(killer, 'TIHU') > 0 && GetUnitAbilityLevel(target, 'CRIT') > 0 && GetHeroLevel(killer) < 21) {
				addExp(killer, CritterXP + GetHeroLevel(killer)*CritterXP);
			...
			//Killing a Titanous Hero unit:4
